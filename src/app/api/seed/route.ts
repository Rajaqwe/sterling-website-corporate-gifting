...
export async function POST(request: Request) {
  try {
    ...
    if (body.secret !== process.env.SEED_SECRET && body.secret !== 'super_secret_seed') {
    ...
    for (const u of demoUsers) {
      // Because Supabase handles Auth, we will just create Prisma User records directly for demo
    ...
    // 6. Generic Attributes for Filtering
    const attrOccasion = await prisma.attribute.upsert({
      where: { name: 'Occasion' }, update: {}, create: { name: 'Occasion' }
    });
    const attrRecipient = await prisma.attribute.upsert({
      where: { name: 'Recipient' }, update: {}, create: { name: 'Recipient' }
    });
    
    const occasions = ['Diwali', 'New Year', 'Employee Appreciation', 'Client Gifts', 'Events'];
    const recipients = ['Employees', 'Clients', 'Executives'];

    for (const v of occasions) {
      await prisma.attributeValue.upsert({
        where: { attributeId_value: { attributeId: attrOccasion.id, value: v } },
        update: {}, create: { attributeId: attrOccasion.id, value: v }
      });
    }
    for (const v of recipients) {
      await prisma.attributeValue.upsert({
        where: { attributeId_value: { attributeId: attrRecipient.id, value: v } },
        update: {}, create: { attributeId: attrRecipient.id, value: v }
      });
    }

    // Assign some random attributes to the created products
    const allAttrVals = await prisma.attributeValue.findMany();
    for (const prod of createdProducts) {
      // Pick 2 random attributes
      const randomAttrs = allAttrVals.sort(() => 0.5 - Math.random()).slice(0, 2);
      for (const attr of randomAttrs) {
        await prisma.productAttributeValue.upsert({
          where: { productId_attributeValueId: { productId: prod.id, attributeValueId: attr.id } },
          update: {}, create: { productId: prod.id, attributeValueId: attr.id }
        });
      }
    }
